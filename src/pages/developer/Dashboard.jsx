...
import {
  FolderKanban, Plus, Users, DollarSign, TrendingUp, Globe
} from 'lucide-react';
...
const DeveloperDashboard = () => {
  ...
  return (
    <div className="pt-20 pb-12 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
      ...
      <div className="bg-slate-900/70 backdrop-blur-md border border-slate-700 rounded-2xl p-8 shadow-2xl">
      ...
      {/* Projects Table */}
      <div className="bg-slate-900/70 backdrop-blur-md border border-slate-700 rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-8 border-b border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          ...
          <button className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-3 rounded-xl font-medium flex items-center gap-2 transition transform hover:scale-105 active:scale-95 shadow-lg shadow-emerald-900/20">
            <Plus size={18} />
            New Project
          </button>
        </div>
                ...
                <tr key={p.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="p-6">
                    <span className={`inline-block px-4 py-1.5 rounded-full text-xs font-medium border ${
                      p.status === 'APPROVED' ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' :
                      p.status === 'PENDING_REVIEW' ? 'bg-amber-500/15 text-amber-400 border-amber-500/30' :
                  ...
                  <td className="p-6">
                    <div className="w-40 h-2 bg-slate-700 rounded-full overflow-hidden">
                      <motion.div 
                        initial={{ width: 0 }}
                        animate={{ width: `${(p.sharesSold / p.totalShares) * 100}%` }}
...
const StatCard = ({ title, value, icon: Icon, color }) => (
  <div className="bg-slate-900/70 backdrop-blur-md border border-slate-700 rounded-2xl p-6 hover:border-slate-600 transition-all duration-300 shadow-lg hover:shadow-xl">
